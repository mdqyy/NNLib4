...
BOOST_AUTO_TEST_CASE(TestAbsModule)
{
	...
	float expected_output[] = {1, 2, 1, 2, 4, 2,  1,  3};
	
	AbsModule<float> a("module1");
	...
	float derivative_expected_output[] = {0,0,1, 2, -1, -2, 4, 2,  1,  3, 0, 0};
...
BOOST_AUTO_TEST_CASE(TestAbsGradient)
{
	...
	std::shared_ptr<ParametersInitializer<double>> initializer(new UniformInitializer<double>());
	...
	std::shared_ptr< Module<double> > m2(new AbsModule<double>("module2"));
