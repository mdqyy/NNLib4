...
BOOST_AUTO_TEST_CASE(TestBiasModule)
{
	...
	float expected_output[] = {2, 6, 1, 1, 5,6,3,6};
	...
	BiasModule<float> a("module1", input_case_dims, initializer, regularizer);
	...
	float derivative_expected_output[] = {2, 6, 1, 1, 5,6,3,6};
	float gradient_expected_output[] = {0, 0, 8, 13, 5, 8, 0, 0};
	auto input_gradients = a.bprop(output_tensor, importances);
	BOOST_CHECK(test_equal_arrays(derivative_expected_output, input_gradients->GetStartPtr(), 8));
	...
	BOOST_CHECK(test_equal_arrays(parameters_tensor.data(), parameters_test+2, 4));
...
BOOST_AUTO_TEST_CASE(TestBiasGradient)
{
	...
	std::shared_ptr< Module<double> > mbias1(new LinearModule<double>("module1", case_input_dims,initializer, regularizer));
	std::shared_ptr< Module<double> > mbias2(new BiasModule<double>("module2", case_input_dims,initializer, regularizer));
	std::shared_ptr< Module<double> > m_abs1(new LinearModule<double>("module3", case_input_dims,initializer, regularizer));
	std::shared_ptr< Module<double> > m_abs2(new TanhModule<double>("module4"));
	std::vector< std::shared_ptr< Module<double> > > modules; modules.push_back(mbias1); modules.push_back(m_abs1); modules.push_back(mbias2); modules.push_back(m_abs2);
