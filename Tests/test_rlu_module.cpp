...
BOOST_AUTO_TEST_CASE(TestRluModule)
{
	...
	float expected_output[] = {1, 2, 0, 0, 4, 2,  1,  3};
	
	RectifiedLinearUnitModule<float> a("module1");
	...
	float derivative_expected_output[] = {0,0,1, 2, 0, 0, 4, 2,  1,  3, 0, 0};
...
BOOST_AUTO_TEST_CASE(TestRluGradient)
{
	...
	std::shared_ptr<ParametersInitializer<double>> initializer(new ConstantInitializer<double>(1));
	...
	std::shared_ptr< Module<double> > m2(new RectifiedLinearUnitModule<double>("module2"));
